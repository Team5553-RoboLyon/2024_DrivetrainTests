...
// ------------------------------------------------------------------------------------------
/**
 *	@brief	Calcule les valeurs de pk1 depuis KIN en appliquant un \bJerk nul sur une duree "dt"
 *			Cette fonction fait la meme chose que NLKIN::to(pk1, 0, dt) de maniere plus optimisee.
 ...
 */
// ------------------------------------------------------------------------------------------
void NLKIN::draw(NL2DOCS * p2docs, const NLKIN * pk0)
{
	Nf32 a0, v0, s0,t0,t;
	Nu32 i;
	Nu32 iter = 32;
	...
	if (dt)
	{
		...
		// Couleurs
		if (m_j > 0.0f)
			p.Color0_4f = p2docs->m_color[NL2DOCS_COLOR_1];//NSetColorf(&p.Color0_4f, NCOLOR_PRESET3F_GREEN_SPRING, 1);
		else if (m_j == 0.0f)
			p.Color0_4f = p2docs->m_color[NL2DOCS_COLOR_3];//NSetColorf(&p.Color0_4f, NCOLOR_PRESET3F_YELLOW, 1);
		else
			p.Color0_4f = p2docs->m_color[NL2DOCS_COLOR_4];//NSetColorf(&p.Color0_4f, NCOLOR_PRESET3F_PINK_BRILLIANT, 1);
