...
inline	NBYTE*	NGetArrayIndexPtr(const NARRAY *parray,const Nu32 index){ NErrorIf(index>=parray->Size,NERROR_ARRAY_INDEX_BEYOND_LIMITS);return parray->pFirst+index*parray->ElementSize; }
...
inline	Nu32	NGetArrayPtrIndex(const NARRAY *parray,const NBYTE*ptr){NErrorIf(!NIsValidArrayElementPtr(parray,ptr),NERROR_ARRAY_INDEX_BEYOND_LIMITS);return ( (ptr - parray->pFirst)/parray->ElementSize );}
